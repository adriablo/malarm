...
class AlarmReceiver : BroadcastReceiver() {
    ...
    private fun handleDismiss(context: Context, intent: Intent) {
        ...
        val reArms = alarm != null && alarm.enabled && scheduler.nextTrigger(alarm) != null
        if (alarm != null) scheduler.cancel(alarm, "Dismiss")
        EventLog.log(
            context, EventType.DISMISSED, id.takeIf { it >= 0 }, alarm?.label,
            "Re-armed".takeIf { reArms },
        )
        stopRinging(context)
        // No-op for disabled/deleted one-shots (nextTrigger null).
        if (reArms) scheduler.schedule(alarm!!)
    }
